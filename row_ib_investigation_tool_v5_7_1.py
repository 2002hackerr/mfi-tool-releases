...
class MFIToolApp:
    def __init__(self):
        ...
        self.root.title("MFI Investigation Tool  v5.7.1  |  ROW IB")
        try:
    ...
    def _process(self):
        try:
            ...
            def inv_cb(curr, total):
                ...
                eta = (elapsed / (curr + 1)) * (total - curr)
                eta_str = f"{int(eta)}s" if eta < 3600 else f"{int(eta/60)}m"
                self._set_status(f"Indexing Invoice Search... {int(pct)}% | ETA: {eta_str}", 35 + int(pct * 0.25))

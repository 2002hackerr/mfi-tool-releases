"""
MFI Investigation Tool  v5.8.12  |  ROW IB
==========================================
ROW IB  |  Amazon
Developed by Mukesh
...
class HeaderCorrectionDialog(tk.Toplevel):
    ...
    def __init__(self, parent, corrections, mapping, df_columns, callback):
        ...
        self.title("Column Header Mismatch Detected — v5.8.12")
...
def write_excel(all_blocks, path):
    ...
    for block in all_blocks:
        for rd in block:
            ...
            # Only claiming-level real rows (depth=0, not special marker rows like [CROSS PO])
            if rd.get('depth', 0) != 0:
                continue

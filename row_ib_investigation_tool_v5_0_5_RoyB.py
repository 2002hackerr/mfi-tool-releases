"""
MFI Investigation Tool  v5.0.5  |  Roy B Workflow Version
========================================================
ROW IB  |  Amazon
Developed by Mukesh

CHANGES IN v5.0.5 (Roy B Workflow):
  ...
  ✔ PHASE 2: Position-matching rule reinforced for (SID, PO, ASIN) matching.
  ✔ PHASE 2: Sequential Investigation Rule reinforced for complete accounting.
  ...
  ✔ EXCEL: Maintained single-sheet output.
"""
...
class InvestigationEngine:
    ...
    def _build_level_logic(self, barcode, inv_no, sid, po, asin,
        ...
        # Roy B Accounting Formula: Total Accounted = Shortage + REBNI (Available) + EX (Adjustments)
        accounted_at_level = shortage + rebni_avail + ex_adj

        # ── PHASE 1: DIRECT SHORTAGE GATEWAY ───────────────────────────────────
        # Rule: If Shortage >= PQV Qty -> Direct Claiming Shortage
        if shortage >= rem_pqv > 0 and not remarks:
            ...
            # ── PHASE 4: CROSS PO CHECK (INDICATOR ONLY IN MANUAL MODE) ──────────
        ...
        if sorted_m:
            ...
            if not remarks and accounted_at_level > 0:
                main_mtc_inv = "Short Received"
                main_mtc_qty = fmt_qty(accounted_at_level)
                remarks = f"Accounted for {int(accounted_at_level)} units (Shortage={int(shortage)}, REBNI={int(rebni_avail)}, EX={int(ex_adj)})"
    ...
    def run_auto(self, barcode, inv_no, sid, po, asin, inv_qty, pqv,
        ...
        if state in self.loop_cache and depth > 0:
            # Cache stores (rows, total_accounted) tuple — unpack directly.
            cached_rows, cached_found = self.loop_cache[state]
            return list(cached_rows), cached_found

        # _build_level_logic returns accounted_at_level = shortage + rebni_avail + ex_adj
        rows, actionable, rec_qty, accounted_at_level, new_rem, ex_adj = self._build_level_logic(barcode, inv_no, sid, po, asin, inv_qty, rem_pqv, depth, is_claiming)
        
        # Budgeting: contribution from this level cannot exceed its allocated branch budget
        total_accounted = min(branch_budget, max(0.0, accounted_at_level))
        rem_budget = branch_budget - total_accounted

        # Early exit if budget exhausted or no further branches
        if rem_budget <= 0 or not actionable or 'REBNI' in rows[0].get('remarks','') or rows[0].get('remarks','') == 'SR':
            if rows and total_accounted > 0 and 'Direct Shortage' not in rows[0].get('remarks', ''):
                rows[0]['remarks'] = f"Accounted for {int(total_accounted)} units at this level — Budget Explained"
            if depth > 0: self.loop_cache[state] = (list(rows), total_accounted)
        ...
        for match in actionable:
            ...
            # Recurse: child level accounts for its budget using the same formula
            child_rows, child_accounted = self.run_auto(self.cache_bc.get(n_inv, "[DICES]"), n_inv, n_sid, n_po, n_asin, n_iqty, pqv, depth+1, visited, rem_pqv - total_accounted, False, n_budget)
            rows.extend(child_rows)
        ...
        if (rows and total_accounted > 0 
                and 'Direct Shortage' not in rows[0].get('remarks', '')
                and 'REBNI Available' not in rows[0].get('remarks', '')):
            status = "explained" if total_accounted >= branch_budget else "partially explained"
            rows[0]['remarks'] = f"Accounted for {int(total_accounted)} units of budget {int(branch_budget)} — Branch {status}"
    ...
    def run_cross_po_investigation(self, candidate, case_type, budget, depth=0, visited=None):
        if visited is None: visited = set()
        c_sid = candidate['sid']; c_po = candidate['po']; c_asin = candidate['asin']; c_iqty = candidate.get('inv_qty', 0)
        ...
        for match in unique:
            ...
            if not n_sid:
                rows.append(self._make_row("[DICES]", n_inv, "[ENTER SID]", n_po, n_asin, n_iqty, "", "", "", "Phase 4: SID not found", "", depth+1))
                continue
                
            child_rows, child_acc = self.run_auto(self.cache_bc.get(n_inv, "[DICES]"), n_inv, n_sid, n_po, n_asin, n_iqty, current_rem, depth+1, visited, current_rem, False, min(n_budget, current_rem))
            rows.extend(child_rows)
...
class MFIToolApp:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("MFI Investigation Tool  v5.0.5  |  Roy B Workflow")
    ...
    def _build_ui(self):
        ...
        tk.Label(t, text="v5.0.5 | Roy B Workflow", fg="#8888aa", bg="#16213e", font=("Segoe UI",10)).pack(side="right", padx=16)
    ...
    def _process(self):
        try:
            ...
            if corrections or missing:
                ...
                if confirmed[0] is None: self.root.after(0, lambda: (self.run_btn.config(state="normal"), self.stop_inv_btn.config(state="disabled"), self.stop_sess_btn.config(state="disabled"))); return
                mc = confirmed[0]

"""
MFI Investigation Tool  v5.3.0  |  ROW IB
==========================================
ROW IB  |  Amazon
Developed by Mukesh

CHANGES IN v5.3.0 (over v5.2.0):
  ✔ [FEATURE] Ticket Type Selector: PDTT (full chain) vs REMASH TT (claiming level only)
...
class ManualLevelDialog(tk.Toplevel):
    ...
    def __init__(self, parent, matches, remaining_pqv, branch_budget, callback,
        ...
        # ── Pending Invoices button — always visible ─────────────────────────
        bf2 = tk.Frame(self, bg="#0f0f1a"); bf2.pack(pady=4)
...
class PreviewPanel(tk.Toplevel):
    COLS      = ['Barcode', 'Inv no', 'SID', 'PO', 'ASIN', 'Inv Qty',
                 'Rec Qty', 'Mtc Qty', 'Mtc Inv', 'Remarks', 'Date', 'CP']
    COL_W_PX  = [130, 160, 130, 90, 110, 60, 60, 60, 160, 240, 150, 180]
    ...
    def _build_level_logic(self, barcode, inv_no, sid, po, asin,
        ...
        if sorted_m:
            ...
            if top['mtc_inv'] == clean(inv_no):
                # Self Matching — clear sorted_m to prevent garbage sub-rows
...
def write_excel(all_blocks, path):
    ...
    headers = ["Barcode", "Inv no", "SID", "PO", "ASIN",
               "Inv Qty", "Rec Qty", "Mtc Qty", "Mtc Inv", "Remarks", "Date", "CP"]
    ...
    for i, w in enumerate([18, 22, 18, 12, 14, 9, 9, 9, 26, 42, 22, 36], 1):
...
class MFIToolApp:
    def __init__(self):
        ...
        self.root.title("MFI Investigation Tool  v5.3.0  |  ROW IB")
        try:

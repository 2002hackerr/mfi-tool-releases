...
class MFIToolApp:
    def __init__(self):
        ...
        self.root.title("MFI Investigation Tool  v5.7.6  |  ROW IB")
        try:
    ...
    def _process(self):
        try:
            ...
            # v5.7.6: More robust queue building (lightweight list of dicts)
            self.manual_q, self.map_cols = [r._asdict() for r in df_c.itertuples(index=False)], mc
            ...
            if self.mode_var.get() == "auto":
                ...
                # v5.7.6: Direct call (v5.5.0 style) to avoid UI-thread deadlock
                self._next_man()
    ...
    def _next_man(self):
        ...
        # v5.7.6: Force visibility of the preview panel
        def force_show():
            if self.preview and self.preview.winfo_exists():
                self.preview.deiconify()
                self.preview.lift()
                self.preview.focus_force()
        self.root.after(0, force_show)
    ...
    def _man_step(self):
        try:
            ...
            # FIX: Explicit timeout/guard for build_one_level
            ...
            import traceback
            tb = traceback.format_exc()
            self.root.after(0, lambda: messagebox.showerror("Engine Error", f"{e}\n\n{tb}"))
            self.root.after(0, self._finish)

"""
MFI Investigation Tool  v5.8.0  |  ROW IB
==========================================
ROW IB  |  Amazon
Developed by Mukesh
...
class InvestigationEngine:
    ...
    def __init__(self, rp, rs, rfb, ip, ifb, sid_cb=None):
        ...
        self.user_overrides = {}
        # v5.8.0 Performance: Initialize high-speed caches for O(1) lookups
        self._shipment_rebni_cache = {}
        for (s, p, a), rows in self.rebni_p.items():
            sq = sum(safe_num(r.get('rebni_available', 0)) for r in rows)
            self._shipment_rebni_cache[(s, p)] = self._shipment_rebni_cache.get((s, p), 0.0) + sq
        ...
        for (s, p, a), rows in self.rebni_p.items():
            for r in rows:
                ...
                if cp > 0:
                    self._cp_cache.setdefault((p, a), cp)
                    break

        self._base_inv_qty_cache = {}
        for (s, p, a), entries in self.inv_p.items():
            for entry in entries:
                ino = clean(entry.get('mtc_inv', ''))
                if ino:
                    iq = safe_num(entry.get('inv_qty', 0))
                    if iq > 0: self._base_inv_qty_cache[(ino, a)] = iq
    ...
    def _resolve_inv_qty(self, inv_no, asin, fallback_qty):
        """v5.8.0: O(1) optimized lookup for base invoice quantity."""
        base = strip_scr(inv_no)
        if base == clean(inv_no): return fallback_qty
        return self._base_inv_qty_cache.get((base, clean(asin)), fallback_qty)

    def _get_shipment_rebni(self, sid, po):
        """v5.8.0: O(1) optimized cache lookup for shipment REBNI."""
        return self._shipment_rebni_cache.get((sid, po), 0.0)

    def get_cp(self, sid, po, asin):
        """v5.8.0: Optimized CP lookup with O(1) fallback cache."""
        rows = self.rebni_p.get((extract_sid(sid), clean(po), clean(asin)), [])
        for r in rows:
            cp = safe_num(r.get('item_cost', 0))
            if cp > 0: return cp
        return self._cp_cache.get((clean(po), clean(asin)), 0.0)
...
class MFIToolApp:
    def __init__(self):
        ...
        self.root.title("MFI Investigation Tool  v5.8.0  |  ROW IB")
        try:
    ...
    def _set_status(self, msg, pct=None):
        # v5.8.0: Optimized status updates by removing update_idletasks()
        # This prevents flooding the event queue during fast indexing.
        def update():
    ...
    def _process(self):
        try:
            ...
            # v5.7.5 Performance: Pre-calculate Manual Queue immediately to prevent post-indexing hang
            ...
            self._set_status("Indexing Complete. Initializing Panel...", 100)
    ...
    def _next_man(self):
        ...
        # v5.8.0: Instant transition — call directly without root.after delay

"""
MFI Investigation Tool  v5.9.1  |  ROW IB
==========================================
ROW IB  |  Amazon
Developed by Mukesh
...
CHANGES IN v5.8.11:
  ✔ [PERF] High-speed 'inv_asin_map' implemented in indexers
  ✔ [UI] Numeric shortage calculation for dynamic highlighting
...
class HeaderCorrectionDialog(tk.Toplevel):
    ...
    def __init__(self, parent, corrections, mapping, df_columns, callback):
        ...
        self.title("Column Header Mismatch Detected — v5.9.0")
...
class CrossPODialog(tk.Toplevel):
    """Cross PO Confirmation Dialog — v5.9.0 (restored from v5.3.0 with correct action keys)"""
    CASE_DESCRIPTIONS = {
        "Case 1": (
...
class CorrespondenceDialog(tk.Toplevel):
    def __init__(self, parent, all_rows):
        ...
        self.title("Scenario Selection — Get Correspondence — v5.8.12")
...
class PreviewPanel(tk.Toplevel):
    ...
    def __init__(self, parent):
        super().__init__(parent)
        self._app = None
        self.title("Investigation Preview — v5.8.12 (editable)")
    ...
    def show_correspondence(self):
        ...
        if not all_rows:
            ...
            return
        # Open dialog as a proper window (resizable, centered)
...
def load_rebni(path):
    ...
    # If the file has more columns than standard, pad names list to avoid length mismatch
...
def build_rebni_index(df):
    p, s, fb = {}, {}, {}
    # v5.9.0 Optimization: Use high-speed dictionary records instead of iterrows()
...
def build_invoice_index(df):
    ...
    for row in df.to_dict('records'):
        ...
        # Optimized lookup map: (Invoice, ASIN) -> Inv Qty
        if inv_no and mtc_asin:
            iam[(inv_no, mtc_asin)] = inv_qty
...
class InvestigationEngine:
    ...
    def _resolve_inv_qty(self, inv_no, asin, fallback_qty):
        """Return correct invoice qty using Optimized Lookup (inv_asin_map)."""
        # Optimized lookup first
        qty = self.inv_iam.get((clean(inv_no), clean(asin)))
        if qty is not None and qty > 0:
            return qty
            
        # Fallback to suffix handling
    ...
    def _make_row(self, b, i, s, p, a, iq, rq, mq, mi, rem, d, depth, rtype='dominant', cp_status='', mtc_asin='', mtc_po=''):
        return {
            ...
            'depth':    depth,
            'type':     rtype,
            'cp_status': cp_status, # This will now hold the numeric CP
        }
    ...
    def _build_level_logic(self, barcode, inv_no, sid, po, asin,
                            ...
                            is_manual=False, cross_po_indicator_only=False, initial_cp=0.0):
        ...
        # Priority 1: CP from claims file. Priority 2: CP from REBNI lookup.
        current_cp = initial_cp if initial_cp > 0 else self.get_cp(sid_frag, po, asin)
        cp_display = f"{current_cp:.2f}" if current_cp > 0 else ""

        shortage           = max(0.0, safe_num(inv_qty) - rec_qty)
        # Row IB Accounting Formula
        accounted_at_level = shortage + rebni_avail + ex_adj

        # v5.8.5: Standardized Remark Construction
        if is_claiming or rec_qty < safe_num(inv_qty) or rebni_avail > 0 or ex_adj > 0:
            if ex_adj > 0 and shortage > 0:
                remarks = (f"Found {int(ex_adj)} units of EX adjustments and {int(shortage)} units of shortage "
                           f"(Inv:{int(inv_qty)} Rec:{int(rec_qty)})")
            else:
                rem_parts = []
                if shortage > 0:
                    rem_parts.append(f"Inv Qty:{int(inv_qty)}.Received Qty:{int(rec_qty)}- Shortage of {int(shortage)} Units")
                if rebni_avail > 0:
                    lvl = 'claiming shipment' if is_claiming else 'matching shipment'
                    rem_parts.append(f"REBNI Available = {int(rebni_avail)} units at {lvl} level — Suggest TSP to utilize")
                if ex_adj > 0:
                    rem_parts.append(f"Found {int(ex_adj)} number of X adjustments")
        ...
        if 'REBNI Available' in remarks or remarks == 'SR':
            rows = [self._make_row(barcode, inv_no, sid, po, asin,
                                    inv_qty, rec_qty, "", "", remarks, rec_date, depth, cp_status=cp_display)]
        ...
        if not is_manual or depth == 0: # v5.8.9: Suppress redundant matching indicators in manual mode deeper levels
    ...
    def run_auto(self, barcode, inv_no, sid, po, asin, inv_qty, pqv,
                 ...
                 branch_budget=None, max_depth_override=None, is_manual=False,
                 row_callback=None):
        ...
        rows, actionable, rec_qty, accounted_at_level, new_rem, ex_adj = \
            self._build_level_logic(barcode, inv_no, sid, po, asin,
                                    inv_qty, rem_pqv, depth, is_claiming, is_manual=is_manual)

        # Improvement 1 & 2: Emit rows as they are generated for Live Preview
        if row_callback:
            for r in rows: row_callback(r)

        # Improvement 2: Depth-0 Cross PO Detection for Auto Mode (Note-only)
        if depth == 0 and not is_manual:
            ...
            for c in candidates:
                cp_row = {
                    ...
                    'rec_qty': fmt_qty(c['rec_qty']),
                    'mtc_qty': '', 'mtc_inv': '',
                    'remarks': (f"Phase 4: Auto-detected Cross PO | {c['cross_type']} "
                                f"| Overage={fmt_qty(c['rec_qty'])} units | Note-only for Auto Mode"),
                    'date': c['date'], 'depth': depth, 'type': 'crosspo',
                }
                rows.append(cp_row)
        ...
        # Sequential sibling investigation (Row IB Rule)
        ...
        for match in actionable:
            ...
            child_rows, child_accounted = self.run_auto(
                ...
                False, n_budget,
                max_depth_override=max_depth_override, is_manual=is_manual,
    ...
    def run_mismatch_investigation(self, mismatch_data, budget, depth=0):
        """Improvement 3: Pivot into a full investigation chain for Mismatch/Overage."""
        m_sid  = extract_sid(mismatch_data.get('sid', ''))
        m_po   = clean(mismatch_data.get('po', ''))
        m_asin = clean(mismatch_data.get('asin', ''))
        m_iq   = safe_num(mismatch_data.get('inv_qty', 0))
        
        # Build logical row for this mismatch pivot
        rows, matches, rq, shortage, new_rem, ex_adj = self._build_level_logic(
            "[MISMATCH]", "", m_sid, m_po, m_asin, m_iq, budget, depth=depth, is_claiming=False)
        
        # Return logical building blocks for manual dialog handling in MFIToolApp
        return rows, matches, rq, shortage, new_rem

    def build_one_level(self, b, i, s, p, a, iq, rem, depth=0, is_claiming=True, is_manual=False, initial_cp=0.0):
        rows, matches, rq, shortage, new_rem, ex_adj = self._build_level_logic(
            b, i, s, p, a, iq, rem, depth, is_claiming, cross_po_indicator_only=is_manual, initial_cp=initial_cp)
    ...
    def run_cross_po_investigation(self, candidate, case_type, budget,
        ...
        c_iqty = safe_num(candidate.get('inv_qty', 0))
        ...
        # v5.3.0 logic: use all matches, sorted by mtc_qty
        ...
        shortage           = max(0.0, c_iqty - rec_qty)
...
def write_excel(all_blocks, path):
    ...
    # Softer Red for Shortage (Improvement 3)
    SHORT_FILL = PatternFill("solid", fgColor="FFCCCC")
    SHORT_FONT = Font(bold=True, color="9C0006", name="Calibri", size=10)
    ...
    for block in all_blocks:
        ...
        for rd in block:
            ...
            # Numeric Shortage Coloring Logic
            iq = safe_num(rd.get('inv_qty'))
            rq = safe_num(rd.get('rec_qty'))
            is_short = (iq > 0 and rq < iq)

            if is_short:
                fill, fnt = SHORT_FILL, SHORT_FONT
            elif 'invalid invoice' in rem.lower():
            ...
            for c, h in enumerate(headers, 1):
                ...
                cell = ws.cell(row=curr, column=c,
                               value=final_val if final_val not in (None, '') else None)
                cell.border = BDR
                cell.font   = fnt
                
                # Improvement 3: Only highlight the REMARKS column if there is a shortage
                if is_short:
                    if h == "Remarks":
                        cell.fill = SHORT_FILL
                        cell.font = SHORT_FONT
                elif fill: 
                    cell.fill = fill
            curr += 1
    ...
    # ── Unique Summary Sheet (v5.9.0) ─────────────────────────────────────────
    # Collects all claiming-level rows, deduplicates, computes Missing QTY
    ws2 = wb.create_sheet(title="Unique Summary")

    SUM_HDR_FILL = PatternFill("solid", fgColor="1F4E79")
    SUM_HDR_FONT = Font(color="FFFFFF", bold=True, name="Calibri", size=10)
    SUM_N_FONT   = Font(name="Calibri", size=10)
    SUM_RED_FILL = PatternFill("solid", fgColor="FFE2E2")   # missing > 0
    SUM_GRN_FILL = PatternFill("solid", fgColor="E2F0D9")   # missing == 0
    CTR          = Alignment(horizontal="center", vertical="center")
    LFT          = Alignment(horizontal="left",   vertical="center")
    SUM_BDR      = Border(left=Side(style='thin'), right=Side(style='thin'),
                          top=Side(style='thin'),  bottom=Side(style='thin'))

    sum_headers = ["Barcode", "Inv no", "SID", "PO", "ASIN",
                   "Inv Qty", "Rec Qty", "Missing QTY", "Cost Price", "Shortage Amount"]
    sum_widths  = [20, 14, 18, 12, 14, 10, 10, 14, 12, 18]

    for ci, (h, w) in enumerate(zip(sum_headers, sum_widths), 1):
        cell = ws2.cell(row=1, column=ci, value=h)
        cell.fill, cell.font, cell.border = SUM_HDR_FILL, SUM_HDR_FONT, SUM_BDR
        cell.alignment = CTR
    ...
    for block in all_blocks:
        for rd in block:
            barcode = str(rd.get('barcode', '')).strip()
            # Improvement 5: Broaden Unique Summary logic
            # Includes claiming (depth 0) and all matching shipments (depth > 0)
            if not barcode or (barcode.startswith('[') and barcode != '[INVALID]'):
                continue

            inv_no = str(rd.get('invoice', '')).strip()
            sid    = str(rd.get('sid',     '')).strip()
            po     = str(rd.get('po',      '')).strip()
            asin   = str(rd.get('asin',    '')).strip()
            iq_raw = rd.get('inv_qty', 0)
            rq_raw = rd.get('rec_qty', 0)

            # Unique key includes all critical identifiers to prevent over-deduplication
            key = (barcode, inv_no, sid, po, asin, str(iq_raw), str(rq_raw))
            if key in seen_keys:
                continue
            seen_keys.add(key)

            iq_num = safe_num(iq_raw)
            rq_num = safe_num(rq_raw) if str(rq_raw).strip() not in ('', None) else 0.0
            
            # Implementation 5: Use a helper tag for fill color
            row_fill = SUM_RED_FILL if iq_num > rq_num else SUM_GRN_FILL

            vals = [barcode, inv_no, sid, po, asin,
                    int(iq_num) if iq_num == int(iq_num) else iq_num,
                    int(rq_num) if rq_num == int(rq_num) else rq_num,
                    None, # Missing Qty formula slot
                    safe_num(rd.get('cp_status', 0)),
                    None] # Shortage Amount formula slot

            for ci, val in enumerate(vals, 1):
                if ci == 8: # Missing QTY Column
                    # Improvement 5: Automated Math Formula
                    formula = f"=F{sum_row}-G{sum_row}"
                    cell = ws2.cell(row=sum_row, column=ci, value=formula)
                elif ci == 10: # Shortage Amount Column
                    # New: Calculate Shortage * CP
                    formula = f"=H{sum_row}*I{sum_row}"
                    cell = ws2.cell(row=sum_row, column=ci, value=formula)
                else:
                    cell = ws2.cell(row=sum_row, column=ci, value=val)
                
                cell.font   = SUM_N_FONT
                cell.border = SUM_BDR
                cell.fill   = row_fill
                cell.alignment = CTR if ci >= 6 else LFT
            sum_row += 1
...
class MFIToolApp:
    def __init__(self):
        ...
        self.root.title("MFI Investigation Tool  v5.9.1  |  ROW IB")
        try:
    ...
    def _build_ui(self):
        ...
        tk.Label(t, text="v5.8.12  |  ROW IB",
                 fg="#8888aa", bg="#16213e",
    ...
    def _set_status(self, msg, pct=None):
        # v5.8.0 Fix: Use .after(0) to update UI from background thread safely
        # to avoid flooding the event loop and causing slow indexing.
        self.root.after(0, lambda: self._update_status_ui(msg, pct))

    def _update_status_ui(self, msg, pct):
    ...
    def _process(self):
        try:
            ...
            ip, ifb, iam = build_invoice_index(load_invoice_search(self.inv_path.get()))
            self.engine = InvestigationEngine(rp, rs, rfb, ip, ifb, iam, self._req_sid)
            tot = len(df_c)

            if self.mode_var.get() == "auto":
                # Improvement 1: Initialize PreviewPanel for Auto Mode visibility
                self.preview = PreviewPanel(self.root)
                self.preview._app = self
                ...
                for i, (_, r) in enumerate(df_c.iterrows()):
                    ...
                    self.preview.add_header_row(f"{i+1}/{tot}: {clean(r.get(mc.get('ASIN',''),''))}")
                    
                    # Capture Cost Price from claims sheet if available
                    claimed_cp = safe_num(r.get(mc.get('CP', ''), 0))
                    ...
                    rows, _ = self.engine.run_auto(
                        ...
                        initial_cp=claimed_cp,
                        row_callback=lambda row: (self.preview.add_row(row), self.root.update())
                    )
    ...
    def _next_man(self, force_next=False):
        # The Guard: Investigate any pending Cross PO before moving to next ASIN
        if hasattr(self, '_pending_cross_po') and self._pending_cross_po:
            ...
            def investigate_pending():
                ...
                # Check for siblings or more cross POs
                self._man_step()
            threading.Thread(target=investigate_pending, daemon=True).start()
            return

        # ASIN Conclusion Gateway:
        # Before jumping to next ASIN, check if there are UNINVESTIGATED sibling matches
        if hasattr(self, 'curr_m') and not force_next:
            pending = self._collect_all_pending()
            if pending:
                self.root.after(0, self._show_pending_gateway)
                return
        ...
        self.curr_m = {
            ...
            'block'         : [],
            'processed'     : set(),
            'asin_rendered_levels': set(), # v5.8.5: Prevent duplicate dominant rows in ASIN block
            'is_new_block'  : True,        # v5.8.5: Tag for isolated Excel blue headers
            'rendered'      : False,
            # FIX 10: Always initialize siblings_stack
            'siblings_stack': [],
            # v5.3.0: Track ALL matched invoices seen across every level
            'all_seen_matches': [],
            'initial_cp'    : safe_num(r.get(mc.get('CP', ''), 0)), # Capture CP for manual mode
        }
    ...
    def _man_step(self):
        ...
        rows, matches, rq, n_rem = self.engine.build_one_level(
            m['b'], m['i'], m['s'], m['p'], m['a'],
            m['iq'], m['rem'], m['depth'], is_claiming=(m['depth'] == 0),
            is_manual=True, initial_cp=m.get('initial_cp', 0.0))

        # v5.8.5: Duplicate Suppression & Block Tagging
        # v5.8.11: Use Composite Rendering Key (Depth + SID + PO + ASIN + Invoice)
        l_key = (m['depth'], clean(m['s']), clean(m['p']), clean(m['a']), clean(m['i']))
        if not m['rendered'] and l_key not in m['asin_rendered_levels']:
            # Tag the first row of a new ASIN investigation for Excel block isolation
            if m.get('is_new_block') and rows:
                rows[0]['is_new_block'] = True
                m['is_new_block'] = False

            m['block'].extend(rows)
            for r in rows: self.preview.add_row(r)
            self.root.update() # Force UI to draw the claiming row immediately
            m['asin_rendered_levels'].add(l_key)
            m['rendered'] = True
        elif not m['rendered']:
            # Level already rendered in this ASIN block; only add unique matching sub-rows
            sub_rows = rows[1:] if len(rows) > 1 else []
            for r in sub_rows: self.preview.add_row(r)
            m['rendered'] = True

        # ── CROSS PO TRIGGER — VISIBILITY FIRST ───────────────────────
        # Improvement 1: Check Cross PO at ALL depths where shortage exists
        # Only check Cross PO AFTER the current level row has been rendered.
        if not m.get('cross_po_checked'):
            ...
            if cands:
                ...
                self.root.after(0, lambda: CrossPODialog(
                    self.root, cands, m['i'], m['s'],
                    lambda res: self._handle_cross_po_and_finish(res)))
                return
        ...
        m['rem'] = n_rem
        # v5.8.8: Comparison using Composite Key (Inv, ASIN, PO, Qty)
        ...
        # Issue 2 Fix: If in manual mode and matches exist, don't hard stop on red rows.
        # Show the manual dialog so user can see red row and decide next move.
        if should_stop and not matches:
            # Check for unresolved sibling branches
            if m.get('siblings_stack'):
                ctx = m['siblings_stack'].pop(0)
                # Ensure the context dict has everything needed for the next step
                m.update({
                ...
                # Show the dialog for the next set of siblings instead of investigating automatically
                self.root.after(0, lambda: self._show_dlg(ctx.get('siblings', [])))
                return

            # Final check for pending cross POs before finalizing ASIN
            if hasattr(self, '_pending_cross_po') and self._pending_cross_po:
    ...
    def _handle_res(self, res, matches):
        ...
        # v5.8.8: Use Composite Key for processed tracking
        if match: self.curr_m['processed'].add(self._get_loop_key(match))
        ...
        if res['action'] == 'invalid':
            ...
            # Improvement 3: Pivot into a full investigation chain
            data = res['mismatch_data']
            budget = safe_num(data.get('ovg_qty', 0))
            if budget <= 0: budget = self.curr_m['rem']

            # Generate mismatch root row and actionable matches for the new item
            rows, sub_matches, rq, shortage, new_rem = self.engine.run_mismatch_investigation(
                data, budget, depth=self.curr_m['depth'])
            
            # Record it in the block and preview
            for r in rows:
                self.curr_m['block'].append(r)
                self.preview.add_row(r)
            
            # Update current remainder based on the mismatch budget coverage logic (if any)
            # For now, we continue manual investigation on these new sub_matches.
            if sub_matches:
                self.root.after(0, lambda: self._show_dlg(sub_matches))
            else:
            ...
            # Standard recursion for all ticket types including REMASH
            remaining_siblings = [
                x for x in matches
            ...
            if remaining_siblings:
                parent_stack.append({
                    ...
                    'p'                : self.curr_m['p'],
                    'a'                : self.curr_m['a'],
                    'processed'        : self.curr_m['processed'], # Shared set
            ...
            self.curr_m.update({
                ...
                'depth'          : self.curr_m['depth'] + 1,
                'rendered'       : False,
                'processed'      : self.curr_m['processed'], # PERSIST shared history
    ...
    def _collect_all_pending(self):
        ...
        for inv in all_matches:
            ...
            if k and k not in seen and k not in processed:
                ...
                if hasattr(self, 'engine') and self.engine.user_overrides:
                    override = self.engine.user_overrides.get(clean(k), {})
                    if override:
                        inv = dict(inv)
                        if 'mtc_qty' in override:
    ...
    def _show_pending_invoices_from_dialog(self):
        ...
        def on_pending_result(res):
            if res['action'] == 'investigate':
                inv_match = res['match']
                self.curr_m['processed'].add(self._get_loop_key(inv_match))
                self.root.after(0,
    ...
    def _show_pending_gateway(self):
        """
        Special gateway used by _next_man to ensure visibility of pending loops.
        Unlike the manual 'View All' call, the 'Next ASIN' button here will
        actually trigger the transition to the next claiming item.
        """
        all_pending = self._collect_all_pending()
        if not all_pending: # Backup check
            self._next_man(force_next=True)
            return

        asin_label = self.curr_m.get('a', '')

        def on_gateway_result(res):
            if res['action'] == 'investigate':
                inv_match = res['match']
                self.curr_m['processed'].add(self._get_loop_key(inv_match))
                # Open the standard manual dialog for this specific match
                self.root.after(0, lambda m=inv_match: self._show_dlg([m]))
            elif res['action'] == 'next_asin':
                # User explicitly confirmed they want to skip and move on
                if self.curr_m.get('block'):
                    self.all_blocks.append(self.curr_m['block'])
                self._next_man(force_next=True)
        ...
        self.active_pending_dlg = PendingInvoicesDialog(
            self.root, all_pending, f"{asin_label} (Conclusion Review)", on_gateway_result)
    ...
    def _handle_cross_po_and_finish(self, res):
        # v5.9.0 FIX: Restored from v5.3.0 — triggers MANUAL investigation dialog
        # instead of silently running auto investigation (which bypassed user interaction)
        if res['action'] == 'skip':
            self.all_blocks.append(self.curr_m['block'])
            self._next_man()
            return
        ...
        # Update curr_m to point at the Cross PO candidate → _man_step shows manual dialog
        self.curr_m.update({
            'b'                 : '',
            'i'                 : '',
            's'                 : candidate['sid'],
    ...
    # ── Helpers ───────────────────────────────────────────────────────────────
    def _get_loop_key(self, mtch):
        """v5.8.8: Returns unique key (Inv, ASIN, PO, Qty) for precise tracking."""
        return (clean(mtch.get('mtc_inv', '')), 
                clean(mtch.get('mtc_asin', '')), 
                clean(mtch.get('mtc_po', '')), 
                fmt_qty(mtch.get('mtc_qty', 0)))

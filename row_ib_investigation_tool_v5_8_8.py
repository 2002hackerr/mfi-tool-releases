"""
MFI Investigation Tool  v5.8.8  |  ROW IB
==========================================
ROW IB  |  Amazon
Developed by Mukesh
...
def write_excel(all_blocks, path):
    ...
    for block in all_blocks:
        ...
        for rd in block:
            ...
            for c, h in enumerate(headers, 1):
                val  = rd.get(KM[h], "")
                # v5.8.8: Numeric conversion to prevent "Number Stored as Text" warning
                final_val = val
                if h in ["Inv Qty", "Rec Qty", "Mtc Qty"] and val not in (None, ''):
                    final_val = safe_num(val)
...
class MFIToolApp:
    def __init__(self):
        ...
        self.root.title("MFI Investigation Tool  v5.8.8  |  ROW IB")
        try:

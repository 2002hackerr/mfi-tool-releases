"""
MFI Investigation Tool  v6.2.4  |  ROW IB (Secured Edition)
==========================================
ROW IB  |  Amazon
Developed by Mukesh

CHANGES IN v6.2.4:
- [REMASH] Autonomous Cross PO investigation (Root Shortage found automatically)
- [REMASH] PO-level auto-population from Shipment Master
- [REMASH] Loop repeating detection and prevention
- [GLOBAL] Persistent Dialog Locking (Sticky Lock toggle)
- [GLOBAL] Right-click context menus for all input fields
- [GLOBAL] Standardized "ROW IB (Secured Edition)" branding

CHANGES IN v6.2.3:
  ✔ [UI] Restored Standard ROW IB Logic for ASIN matching.
  ✔ [DATA] Optimized REBNI indexing for high-speed cross-PO searching.
"""
...
class DownloadProgressDialog(tk.Toplevel):
    ...
    def start_download(self):
        try:
            ...
            response.raise_for_status()  # v6.2.4 Security: Verify download success before writing
...
def _custom_toplevel_init(self, *args, **kw):
    ...
    # v6.2.4: Append a floating Pin/Lock toggle to all dialog panels (Globally persistent state)
...
class HeaderCorrectionDialog(tk.Toplevel):
    ...
    def __init__(self, parent, corrections, mapping, df_columns, callback):
        ...
        # v6.2.4 Fix: Updated title version
        self.title("Column Header Mismatch Detected — v6.2.4")
...
class CrossPODialog(tk.Toplevel):
    ...
    def _analyze_cross_po_file(self):
        # v6.1.2: Enhanced dynamic analysis with detailed findings report (Barcode support)
        ...
        try:
            ...
            for c in self.candidates:
                match = df[(df[sid_col].astype(str).str.contains(target_sid)) & (df[po_col].astype(str) == c['po'])]
                if not match.empty:
                    found_inv = clean(str(match.iloc[0][inv_col]))
                    found_bc  = clean(str(match.iloc[0][bc_col])) if bc_col else ""
                    ...
                    if self.engine:
                        ...
                        if iq is not None:
                            ...
                            findings.append(f"• PO {c['po']}: Inv {found_inv} found in file, but Qty not in database")
                else:
                    findings.append(f"• PO {c['po']}: Not found in attached file")
            
            # v6.1.2: Dynamically refresh the UI grid immediately
            self._render_candidates_table()
            
            report = "\n".join(findings) if findings else "No candidates processed."
            messagebox.showinfo('Analysis Results', f'Found {updated_count} matches in file:\n\n{report}', parent=self)
...
class ManualLevelDialog(tk.Toplevel):
    ...
    def _show_pending(self):
        # v6.2.4: Isolated per-ASIN matches
...
class CorrespondenceDialog(tk.Toplevel):
    def __init__(self, parent, all_rows, app=None):
        ...
        self.app = app
        self.title("Scenario Selection — Get Correspondence — v6.2.4")
...
class PreviewPanel(tk.Toplevel):
    ...
    def __init__(self, parent):
        super().__init__(parent)
        self._app = None
        self.title("Investigation Preview — v6.2.4 (editable)")
    ...
    def _ctx_clear(self):
        """v6.2.4: Clear the targeted column across all selected rows."""
        if self._ctx_iid is None or self._ctx_col_idx is None: return
        self._take_snapshot()
        col_name = self.COLS[self._ctx_col_idx]
        sel = self.tree.selection()
    ...
    def _ctx_insert_row(self):
        """v6.2.4: Insert an entirely blank row immediately below the selected row."""
    ...
    def confirm_edits(self):
        ...
        for iid in self.tree.get_children():
            ...
            # v6.2.4: Auto-recalculate Remarks if Inv Qty and Rec Qty create a math discrepancy
        ...
        # v6.2.4: Synchronize edits across ALL active dialogs globally
        updated_any = False
...
def load_invoice_search(path):
    ...
    df.columns = names[:len(df.columns)]
    # v6.2.4 Deduplication: Strictly remove redundant invoice-asin-po-sid records
...
def build_shipment_index(df):
    """v6.2.4: Builds an index from (SID, PO) to (Barcode, Invoice) for auto-population."""
    idx = {}
...
class InvestigationEngine:
    ...
    def __init__(self, rp, rs, rfb, rsid, ip, ifb, iam, sid_cb=None, ship_idx=None):
        ...
        self.inv_iam    = iam
        self.sid_cb     = sid_cb
        self.shipment_index = ship_idx or {} # v6.2.4: Shipment Master index for barcodes
        ...
        self.asin_pending_matches = [] # v6.2.4: Per-ASIN match isolation
    ...
    def _build_level_logic(self, barcode, inv_no, sid, po, asin, iqty, rem_pqv, depth, is_claiming, is_manual=False, cross_po_indicator_only=False, initial_cp=0.0):
        ...
        # v6.2.4: Auto-populate barcode and invoice if missing using (SID, PO) composite index
        ...
        if shortage >= rem_pqv > 0 and not remarks:
            ...
            if cross_po_indicator_only and self.ticket_type != "REMASH":
                ...
                # v6.2.4: For REMASH, always automate Cross PO traversal even in manual/indicator modes
    ...
    def run_mismatch_investigation(self, data, budget, depth=0):
        # v6.2.4 Fix: Properly unpack all 6 variables and return them for consistent UI handling
    ...
    def build_one_level(self, b, i, s, p, a, iq, rem, depth=0, is_claiming=True, is_manual=False, initial_cp=0.0):
        # v6.2.4: Consistent return with 6-value internal engine return
    ...
    def run_cross_po_investigation(self, c, case_type, budget, depth=0, visited=None):
        ...
        for match in unique:
            ...
            # REMASH SID check
            # v6.2.4: Cross PO investigation always allows shipment transitions to trace overage chains
            pass 
...
class SearchCloudDialog(tk.Toplevel):
    def __init__(self, parent, s_type, callback):
        ...
        self.callback = callback
        self.s_type = s_type
        self.title(f"Search {s_type} — v6.2.4")
...
class MFIToolApp:
    def __init__(self):
        ...
        self.root = tk.Tk(); self.root.title("MFI Investigation Tool  v6.2.4  |  ROW IB (Secured Edition)")
    ...
    def _setup_global_context_menu(self):
        """v6.2.4: Provides right-click Copy/Paste/Cut context menus globally across all inputs."""
        ...
        # v6.2.4: OTA Auto-Update Check on Startup
    ...
    def _build_ui(self):
        ...
        tk.Label(t, text="v6.2.4 | ROW IB (Secured Edition)", fg="#f0a500", bg="#16213e", font=("Segoe UI", 10, "bold")).pack(side="right", padx=10)
        ...
        self.run_btn = tk.Button(bf, text="▶ RUN", bg="#e94560", fg="white", font=("Segoe UI", 9, "bold"), padx=10, pady=5, relief="flat", cursor="hand2", command=self.start_run); self.run_btn.pack(side="left", padx=3)
        self.stop_inv_btn = tk.Button(bf, text="⏸ STOP", bg="#4a2020", fg="white", font=("Segoe UI", 9, "bold"), padx=8, pady=5, relief="flat", state="disabled", cursor="hand2", command=self.request_stop_investigation); self.stop_inv_btn.pack(side="left", padx=3)
    ...
    def reset_tool(self):
        ...
        self.rebni_path.set("")
        self.inv_path.set("")
        self.ship_master_path.set("") # v6.2.4 Fix: Clear Shipment Master on reset
    ...
    def _exec_cloud_query(self, s_type, params):
        try:
            ...
            # Map filenames for Local/Cloud per recovery logic
            f_map = {
            ...
            # Save to Downloads path with strict naming pattern
    ...
    def _process(self):
        try:
            ...
            # v6.2.4: Shipment Master Integration
            ship_idx = {}
            ...
            if self.mode_var.get() == "auto":
                ...
                for i, (_, r) in enumerate(df_c.iterrows()):
                    ...
                    # v6.2.4: Reset ASIN isolation state
    ...
    def restore_panels(self):
        # v6.1.2 Fixed: Restore any panel that was minimized or explicitly hidden (withdrawn)
        options = []
        # v6.1.2: Strictly check for 'withdrawn' or 'iconic' (minimized) state.
        # This excludes 'normal' and 'zoomed' (maximized) states which are visible.
        ...
        if not options:
            # v6.1: If preview was destroyed but we have data, recreate it
            if self.all_blocks:
                self.preview = PreviewPanel(self.root); self.preview._app = self
                ...
                self._set_status('Preview panel restored from saved data.')
                return
            self._set_status('No panels available to restore.')
            return

        if len(options) == 1:
            # v6.1: Only one panel — restore it directly without showing a dialog
            self._do_restore(options[0])
            return

        # v6.1: Multiple panels — show selection dialog
        dlg = tk.Toplevel(self.root)
        dlg.title('Restore Panel')
        dlg.geometry('380x220')
        dlg.configure(bg='#16213e')
        dlg.resizable(False, False)
        tk.Label(dlg, text='Select panel to restore:', bg='#16213e', fg='white',
                 font=('Segoe UI', 11, 'bold')).pack(pady=(15, 8))
        v = tk.StringVar(value=options[0])
        cb = ttk.Combobox(dlg, textvariable=v, values=options, state='readonly', width=38,
                          font=('Segoe UI', 10))
        cb.pack(pady=8)
        def do_restore():
            self._do_restore(v.get())
            dlg.destroy()
        tk.Button(dlg, text='✔  Restore Selected', command=do_restore,
                  bg='#2d6a4f', fg='white', font=('Segoe UI', 10, 'bold'),
                  padx=16, pady=8, relief='flat', cursor='hand2').pack(pady=12)
    ...
    def _do_restore(self, panel_name):
        # v6.1: Actually bring back a specific panel — deiconify + lift + focus
    ...
    def _next_man(self, force_next=False):
        ...
        if hasattr(self, 'curr_m') and not force_next:
            ...
            # v5.9.3 fix: Commit the completed block to all_blocks ONLY when moving to the next ASIN
        ...
        # v6.2.4: Reset isolation per ASIN when moving to the next item
        self.engine.asin_pending_matches = []
        self.global_processed.clear()
    ...
    def _man_step(self):
        ...
        for mt in matches:
            ...
            # v6.2.4: Deduplicate identical invoice matches from the manual dialog entirely.
    ...
    def _handle_res(self, res, matches):
        ...
        if res['action'] == 'invalid':
            ...
            # v6.2.4 Fix: Signature mismatch (Added 'ex' to unpack 6 returns)
    ...
    def _collect_all_pending(self):
        ...
        # v6.1.1: Pull from the engine's global net instead of UI limits
        src = self.engine.all_found_matches if hasattr(self, 'engine') and hasattr(self.engine, 'all_found_matches') else self.curr_m.get('all_seen_matches', [])
        for inv in src:
    ...
    def _handle_cross_po(self, res):
        ...
        # v6.1.2: Preserve found Barcode and Invoice for pending items
    ...
    def open_summary_portal(self):
        """v5.9.4 Integration: Automatically launch the embedded HTML portal."""
        # ── v5.9.4 BUNDLING LOGIC ───────────────────────────────────────────

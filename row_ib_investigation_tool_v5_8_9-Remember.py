...
class PreviewPanel(tk.Toplevel):
    ...
    def __init__(self, parent):
        ...
        self.title("Investigation Preview — v5.9.0 (editable)")
        ...
        for tag, bg, fg in [
            ...
            ('shortage_red', '#ffcccc', '#9c0006'), # Softer red (Improvement 3)
        ]:
    ...
    def add_row(self, rd):
        ...
        # Universal Red Highlighting logic: Received < Invoiced
        is_short = (iq > 0 and rq < iq)
        remarks  = str(rd.get('remarks', '')).lower()
        
        tag = ('shortage_red' if is_short else
    ...
    def confirm_edits(self):
        ...
        for iid in self.tree.get_children():
            ...
            if override:
                ...
                # Improvement 4: Synchronize Remarks with the new manual math
                new_iq = override.get('inv_qty', safe_num(original_inv_qty))
                new_rq = override.get('mtc_qty', safe_num(rd.get('rec_qty', 0))) # in preview, rec_qty is often reused for mtc_qty
                
                # If Rec Qty column was also edited, pull it from the current row data
                current_rec_col = str(d.get('Rec Qty', '')).strip()
                if current_rec_col:
                    new_rq = safe_num(current_rec_col)

                if new_iq > new_rq:
                    new_rem = f"Inv Qty:{int(new_iq)}.Received Qty:{int(new_rq)}- Shortage of {int(new_iq - new_rq)} Units (Edit Confirmed)"
                    
                    # Update Treeview immediately
                    vals = list(self.tree.item(iid, 'values'))
                    vals[11] = new_rem # Remarks column
                    self.tree.item(iid, values=vals)
                    # Update internal data
                    self._row_data[iid]['Remarks'] = new_rem
                    if '_rd' in self._row_data[iid]:
                        self._row_data[iid]['_rd']['remarks'] = new_rem
...
class InvestigationEngine:
    ...
    def _build_level_logic(self, barcode, inv_no, sid, po, asin,
                            ...
                            is_manual=False, cross_po_indicator_only=False):
...
def write_excel(all_blocks, path):
    ...
    sum_headers = ["Barcode", "Inv no", "SID", "PO", "ASIN",
                   "Inv Qty", "Rec Qty", "Missing QTY"]
    sum_widths  = [20, 14, 18, 12, 14, 10, 10, 14]
    ...
    for block in all_blocks:
        for rd in block:
            ...
            inv_no  = str(rd.get('invoice', '') or rd.get('mtc_inv', '')).strip()
            sid     = str(rd.get('sid',     '')).strip()
            po      = str(rd.get('po',      '') or rd.get('mtc_po', '')).strip()
            asin    = str(rd.get('asin',    '') or rd.get('mtc_asin', '')).strip()
            iq_raw  = rd.get('inv_qty', 0)
            rq_raw  = rd.get('rec_qty', rd.get('mtc_qty', 0))

            # Composite Signature (7 keys) - ASIN ensures uniqueness for multi-ASIN shipments
            key = (barcode, inv_no, sid, po, asin, safe_num(iq_raw), safe_num(rq_raw))
            if key in seen_keys:
                continue
            seen_keys.add(key)
            ...
            vals = [barcode, inv_no, sid, po, asin,
                    ...
                    None] # Missing Qty slot for formula
...
class MFIToolApp:
    def __init__(self):
        ...
        self.root.title("MFI Investigation Tool  v5.9.0  |  ROW IB")
        try:

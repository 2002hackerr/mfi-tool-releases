"""
MFI Investigation Tool  v5.8.10  |  ROW IB
==========================================
ROW IB  |  Amazon
Developed by Mukesh
...
def write_excel(all_blocks, path):
    ...
    for block in all_blocks:
        ...
        for rd in block:
            ...
            for c, h in enumerate(headers, 1):
                val  = rd.get(KM[h], "")
                # v5.8.8/5.8.9: Universal Numeric conversion to prevent ALL "Number Stored as Text" warnings
                final_val = val
                str_val   = str(val).strip()
                if val not in (None, ''):
                    # Try converting purely numeric strings to int/float (SID, Barcode, etc.)
...
class MFIToolApp:
    def __init__(self):
        ...
        self.root.title("MFI Investigation Tool  v5.8.10  |  ROW IB")
        try:
    ...
    def _build_ui(self):
        ...
        tk.Label(t, text="v5.8.10  |  ROW IB",
                 fg="#8888aa", bg="#16213e",
    ...
    def _man_step(self):
        ...
        if should_stop:
            ...
            while stack:
                ctx      = stack.pop()
                # v5.8.8: Composite Key comparison
                ...
                if siblings:
                    ...
                    m.update({
                        ...
                        'p'               : ctx['p'],
                        'a'               : ctx['a'],
                        'processed'       : m['processed'], # Inherit shared processed set
            ...
            if all_pending:
                # v5.8.9: AUTO-sequential investigation flow for cached SIDs
                first_p = all_pending[0]
                if first_p['mtc_inv'] in self.engine.cache_sid:
                    # Silently mark as processed and launch results directly (No popup!)
                    m.get('processed', set()).add(self._get_loop_key(first_p))
                    self._show_dlg([first_p])
                    return

                # Show dialog listing all uninvestigated matched invoices (Only for new ones)
                asin_label = m.get('a', '')
                def on_pending_result(res):
                    if res['action'] == 'next_asin':
                        ...
                        # v5.8.8: Add Composite Key to processed set
                        m['processed'].add(self._get_loop_key(inv_match))

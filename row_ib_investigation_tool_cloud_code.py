"""
MFI Investigation Tool  v5.0.2
==============================
ROW IB  |  Amazon
Developed by Mukesh

CHANGES IN v5.0.2 (over v5.0.1):
  ✔ Added strip_scr() helper — strips trailing SCR/SCRSCR/SCRSCRSCR from invoice numbers
  ✔ Added _resolve_inv_qty() — returns correct invoice qty for SCR-suffixed invoices
    by looking up the base invoice number+ASIN in the invoice index
  ✔ run_auto: uses _resolve_inv_qty for n_iqty before recursing into sub-invoices
  ✔ _build_level_logic: resolves inv_qty for actionable matches (SCR fix at match level)
  ✔ _handle_res: stores pending_siblings when user picks one branch from ManualLevelDialog
  ✔ _man_step: resumes investigation of pending sibling branches after each branch completes
    (fixes premature block finalization — all matched invoices now investigated in full)
"""
...
class HeaderCorrectionDialog(tk.Toplevel):
    ...
    def __init__(self, parent, corrections, mapping, df_columns, callback):
        ...
        self.title("Column Header Mismatch Detected — v5.0.2")
...
class InvestigationEngine:
    ...
    def run_cross_po_investigation(self, candidate, case_type, budget, depth=0, visited=None):
        ...
        raw = self._inv_lookup(c_sid, c_po, c_asin); seen = set(); unique = []
        for m in raw:
            combo = (m['mtc_inv'], m['mtc_po'], m['mtc_asin'])
        ...
        for match in unique:
            ...
            n_inv = match['mtc_inv']; n_po = match['mtc_po']; n_asin = match['mtc_asin']; n_iqty = match['inv_qty']; n_budget = safe_num(match['mtc_qty'])
...
class MFIToolApp:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("MFI Investigation Tool  v5.0.2  |  ROW IB")
    ...
    def _build_ui(self):
        ...
        tk.Label(t, text="v5.0.2  |  ROW IB", fg="#8888aa", bg="#16213e", font=("Segoe UI",10)).pack(side="right", padx=16)
    ...
    def _man_step(self):
        ...
        should_stop = (not matches or
                       any(x in rem_str for x in
                           ["Root cause", "REBNI", "SR", "Found", "short received directly"]))
        ...
        if should_stop:
            ...
            # ── Check for pending sibling branches (other matches from parent level) ──
            # When user picked one branch (e.g., A1) from ManualLevelDialog showing
            # [A1, A2, A3], A2 and A3 were stored in 'pending_siblings'.
            # After A1's chain completes here, show dialog for A2 before finalizing.
            pending = m.get('pending_siblings', [])
            if pending:
                # Remove first pending sibling and show dialog for it
                next_sibling = pending[0]
                m['pending_siblings'] = pending[1:]
                # Only proceed if not already processed
                if next_sibling['mtc_inv'] not in m.get('processed', set()):
                    self.root.after(0, lambda sib=next_sibling: self._show_dlg([sib] + m.get('pending_siblings', [])))
                    return

            # ── No more siblings and no Cross PO → finalize ─────────────────────
    ...
    def _handle_res(self, res, matches):
        ...
        if res['action'] == 'invalid':
            ...
            # Store sibling matches not yet investigated so we can resume them
            # after this branch chain finishes.
            ...
            self.curr_m.update({
                ...
                'a'                : match['mtc_asin'],
                'iq'               : match['inv_qty'],
                'budget'           : safe_num(match['mtc_qty']),
                'depth'            : self.curr_m['depth'] + 1,
                'rendered'         : False,
                'processed'        : set(),
                'pending_siblings' : remaining_siblings,   # ← siblings to investigate next
            })
